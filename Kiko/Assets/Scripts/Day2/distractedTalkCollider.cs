...
public class distractedTalkCollider : MonoBehaviour
{
    ...
    {
        if (other.tag == "Player" && day2Triggers.hasTalkedTopher == false)
        {
            dRunner.StartDialogue("distractedTrigger");
            moveControl.DisableMovement();
            Debug.Log("Distracted collider Active");
        }

        if (other.tag == "Player" && day2Triggers.hasTalkedTopher == true)
        {
